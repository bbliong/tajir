import React from 'react'

import IconTraveler from 'assets/images/icons/ic_traveler.svg'
import IconCities from 'assets/images/icons/ic_cities.svg'
import IconTreasure from 'assets/images/icons/ic_treasure.svg'
import ImageBanner from 'assets/images/banner.png'
import ImageFrameBanner from 'assets/images/frame.png'
import Button from 'element/button'
import numberFormat from 'utils/formatNumber'

export default function Hero(props) {
    let showMostPicked = () => {
        window.scrollTo({
            top: props.refMosPicked.current.offsetTop - 30,
            behavior: "smooth"
        })
    }

    return (
        <section className="container pt-4">
            <div className="row align-items-center">
                <div className="col-auto pr-5" style={{ width: 560 }}>
                    <h1 className="h2 font-weight-bold line-height-1 mb-3">
                        Forget Busy Work <br />
                            Start Next Vacation
                        </h1>
                    <p className="mb-5 font-weight-light text-gray-500 w-75">
                        We provide what you need to enjoy your holiday with family. Time to make another memorable moments.
                        </p>
                    <Button className="btn px-5" href="" type="link" hasShadow isPrimary onClick={showMostPicked}>Show Me Now</Button>
					<div className="row mt-5">
						<div className="col-auto" style={{marginRight: 50}}>
							<img src={IconTraveler} alt={`${props.data.hero.travelers}`}></img>
							<h6 className="mt-3">
								{numberFormat(props.data.hero.travelers)}{" "}
								<span className="text-gray-500 font-weight-light">
									Travelers
								</span>
							</h6>
						</div>
						<div className="col-auto" style={{marginRight: 50}}>
							<img src={IconTreasure} alt={`${props.data.hero.treasures}`}></img>
						<h6 className="mt-3">
							{numberFormat(props.data.hero.treasures)}{" "}
							<span className="text-gray-500 font-weight-light">
								Treasure
							</span>
						</h6>
						</div>
						<div className="col-auto" style={{marginRight: 50}}>
							<img src={IconCities} alt={`${props.data.hero.cities}`}></img>
						<h6 className="mt-3">
							{numberFormat(props.data.hero.cities)}{" "}
							<span className="text-gray-500 font-weight-light">
								Cities
							</span>
						</h6>
					</div>
					</div>
				</div>
				<div className="col-5 pl-5">
						<div style={{ width: 540, height: 410}}>
							<img
								src={ImageBanner}
								alt="Room WIth Couchers"
								className="img-fluid position-absolute"
								style={{margin : "-30px 0 0 -30px", zIndex : 1}}
							></img>
							<img
								src={ImageFrameBanner}
								alt="Room WIth Couchers"
								className="img-fluid position-absolute"
								style={{margin : "0 -15px -15px 0", zIndex : 1}}
							></img>
						</div>
					</div>	
			</div>
        </section>
    )
}
